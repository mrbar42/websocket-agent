...
Agent.conf = function (arg) {
    ...
    switch (typeof arg) {
            ...
            promise = Promise
                ...
                .then(function (identity) {
                    if (identity && typeof identity == 'object') {
                        setTimeout(function () {
                            Agent.emit('_newIdentity', identity);
                        }, 1);
                ...
                .catch(function (err) {
                    if (err == Agent.TIMEOUT) {
                        Log.e("Failed to get identity data. (15s timeout)");
                    }
                    else {
                        Log.e("Failed to get identity data. promise failed", err);
                    }
                    process.exit(1);
                });
            break;
        case 'undefined':
            ...
            break;
        default:
            console.error("Agent: Called identity method with invalid argument [%s] %s", typeof arg, arg);
            process.exit(1);
            break;
    }
